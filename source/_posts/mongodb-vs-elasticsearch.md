...
一个多月没有更新博客了，再次印证了我是一个爱偷懒的程序员(/笑脸)

2017年已过去，如果用一句话来总结自己的2017年，那将是：收获爱情，提升职能的一年。总体来说过去的一年不算差，也不算太好。没有什么大风大浪，事业和爱情稳步进行着...

2018年的期望是自己能在职业道路上有所突破，其它的事儿也要一切顺利。

### 业务场景

笔者从以mongodb为主要业务数据库做开发到现在应该有1年多的时间，随着公司的业务数据量逐渐积累，现在也已经引进了性能更强大的elasticsearch作为业务数据库。

笔者最近正是在做elasticsearch数据库相关的业务开发，因此对于两者的特点有了一些新的看法。
...
elasticsearch有一个较为复杂的索引分析器，每一次搜索会有一个相对较于普通索引方式更复杂的过程，但其也有缓存机制。并发性能方面应该不会比mongodb强。

### 聚合性能

mongodb聚合分析会遍历符合筛选条件的文档，故其复杂度不会低于O(n)。

elasticsearch的倒排索引对于聚合有绝对性优势，聚合分析并不一定会遍历每一条数据，复杂度可能会小于O(n)。

### 总结

mongodb的并发查询性能较好，适合做高并发量业务后端数据库。elasticsearch对于聚合分析处理的性能极好，对于海量数据聚合分析优选该数据库。
